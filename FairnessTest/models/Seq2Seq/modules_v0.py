...
class Seq2seq(nn.Module):
    ...
    def __init__(self, opt, num_features,
        ...
        shared_lt = (self.decoder.lt if opt['lookuptable'] in ['enc_dec', 'all'] else None)
    ...
    def forward(self, xs, ys=None, cands=None, valid_cands=None, prev_enc=None,
        ...
        # print("hidden: {}".format([h.size() for h in hidden]))
    ...
    def forward_with_contenxt_vector(self, cv, ys):
        hidden = (cv[0], cv[1])
        bsz = 1
        self.longest_label = max(self.longest_label, ys.size(1))
        ...
        preds, score, hidden = self.decoder.forward_without_attention(xs, hidden)
        predictions.append(preds)
        scores.append(score)
    ...
    def forward_with_contenxt_vector_for_predict(self, cv, beam_size=1, topk=1):
        '''

        :input: context vector
        :output: predctive response
        '''
        hidden = (cv[0], cv[1])
        bsz = 1
...
class Decoder(nn.Module):
    ...
    def forward(self, xs, hidden, encoder_output, attn_mask=None, topk=1):
        ...
        if self.attn_time == 'post':
            output = self.attention(output, new_hidden, encoder_output, attn_mask) # output = output
