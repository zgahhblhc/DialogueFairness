...
class Seq2seqAgent(Agent):
    ...
    def __init__(self, opt, shared=None):
        ...
        if shared:
            ...
            # print("os.path.isfile(opt['model_file']): ", os.path.isfile(opt['model_file']))
    ...
    def decoder_backward(self, cv, ys):

        def decoder_loss(score, ys):
            max_score = score.max(1)[0]
            ys_score = torch.squeeze(torch.gather(score, 1, ys.unsqueeze(1)), 1)

            # print("score: ", score)
            # print("max_score: ", max_score)
            # print("ys: ", ys)
            # print("ys_score: ", ys_score)
            loss = torch.sum(max_score - ys_score)
            # loss = torch.sum(- ys_score)
        ...
        out = None
        try:
            _preds, scores = self.model.forward_with_contenxt_vector(cv, ys)

            score_view = scores.view(-1, scores.size(-1))
            loss = decoder_loss(score_view, ys.view(-1))
    ...
    #
    # # def vectorize_for_LM(self, samples):
    # #     """Convert a list of observations into input & target tensors."""
    # #     # print("observations: ", observations)
    # #     is_training = False
    # #     xs, ys, labels, valid_inds, _, _ = PaddingUtils.pad_text_for_LM(
    # #         samples, self.dict, end_idx=self.END_IDX,
    # #         null_idx=self.NULL_IDX, dq=True, eval_labels=True,
    # #         truncate=self.truncate)
    # #
    # #
    ...
    # #     return xs, ys, labels, valid_inds, cands, valid_cands, is_training
    #
    # def vectorize_fa(self, target, add_eos=True):
    #     """Convert a list of observations into input & target tensors."""
    #
    #     ys = [deque(self.dict.txt2vec(target))]
    #
    #
    #     if ys is None:
    #         return None
    ...
    #     if add_eos:
    #         if self.use_cuda:
    #             ys = torch.cat((ys, torch.tensor([[2]]).cuda()), 1)
    #         else:
    #             ys = torch.cat((ys, torch.tensor([[2]])), 1)
    #     # print("ysss: ", ys)
    #
    #     return ys

    def vectorize(self, targets):
        """Convert a list of observations into input & target tensors."""
        ys = [self.dict.txt2vec(target) for target in targets]

        assert self.dict.tok2ind[self.dict.null_token] == 0

        max_length = max([len(y) for y in ys])

        for i in range(len(ys)):
            ys[i] += [0] * (max_length - len(ys[i]))
    ...
    def encode_attack(self, xes, target_cv):
        # xes = self.model.encoder.dropout(xes)
        # print("xes: ", xes.size())

        _, hidden = self.model.encoder.rnn(xes)
        current_cv = torch.stack((hidden[0], hidden[1]), dim=0)
        # print("current_cv: ", current_cv.size())
        if target_cv is not None:
            loss = torch.norm(current_cv - target_cv)

            return loss, current_cv
        else:
            return current_cv

    def decode_attack(self, cv, target):

        ys = self.vectorize_fa(target)

        loss, preds = self.decoder_backward(cv, ys)

        pred_txt = self.dict.vec2txt(vector=preds[0])

        return loss, pred_txt
